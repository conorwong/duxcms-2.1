...
class FieldsetModel extends Model {
    ...
    public function saveData($type = 'add' ,$autoKey = false){
        ...
        if($type == 'add'){
            //创建数据表
            $sql=" CREATE TABLE IF NOT EXISTS `__PREFIX__ext_".$data['table']."` ( ";
            if($autoKey){
                $sql .= '
        ...
        if($type == 'edit'){
            ...
            //修改数据表
            $sql="
            ALTER TABLE __PREFIX__ext_".$info['table']." RENAME TO __PREFIX__ext_".$data['table']."
            ";
            $statusSql = $this->execute($sql);
            if($statusSql === false){
                return false;
    ...
    public function delData($fieldsetId)
    {
        ...
        //删除数据表
        $sql="
            DROP TABLE `__PREFIX__ext_".$info['table']."`
        ";
        $statusSql = $this->execute($sql);
        if($statusSql === false){
            return false;
    ...
    public function getInfoClassId($classId)
    {
        ...
        //获取完整栏目信息
        $classInfo=D($classInfo['app'].'/Category'.$classInfo['app'])->getInfo($classId);
